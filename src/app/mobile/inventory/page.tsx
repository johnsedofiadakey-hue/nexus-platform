...
import { Search, Package, AlertTriangle, Loader2, Zap, RefreshCw } from "lucide-react";
...
export default function InventoryPage() {
  ...
  const fetchInventory = async () => {
    setLoading(true);
    try {
      // Timestamp 't' ensures we get fresh data every time (no caching)
      const res = await fetch("/api/inventory?t=" + Date.now());
      
      if (res.ok) {
        const data = await res.json();
        // ✅ FIX: The new API returns the array directly, not inside 'items'
    ...
    <div className="min-h-screen bg-slate-50 p-4 pb-24">
      ...
      <div className="bg-white p-4 rounded-3xl shadow-sm border border-slate-200 mb-6 sticky top-4 z-10">
        <div className="flex justify-between items-center mb-3">
           ...
           <button onClick={fetchInventory} className="p-2 bg-slate-50 rounded-full text-blue-600 hover:bg-blue-50">
      ...
      <div className="space-y-3">
        {filteredProducts.length > 0 ? (
          filteredProducts.map((product) => {
            ...
            const isFastMoving = product.salesVelocity > 5; // Logic from Dashboard
