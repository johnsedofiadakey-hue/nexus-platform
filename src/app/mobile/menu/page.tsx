...
export default function MobileMenu() {
  ...
  const handleSubmitLeave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setView('LEAVE_LIST'); // Return to list
        setLeaveForm({ type: "Annual Leave", startDate: "", endDate: "", reason: "" }); // Reset
      } else {
  ...
  if (view === 'MENU') {
    return (
      <div className={`min-h-[80vh] transition-colors duration-500 pb-24`}>
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className={`p-6 rounded-[2rem] border shadow-sm space-y-6 ${themeClasses.card} ${themeClasses.border}`}>
               <div className="flex items-center justify-between">
                 ...
                 <button onClick={toggleDarkMode} className={`w-12 h-6 rounded-full p-1 transition-colors ${darkMode ? `bg-${accent}-600` : "bg-slate-200"}`}>
               ...
               <div className="flex items-center justify-between">
                 <div className="flex items-center gap-3">
                   ...
                   <p className={`font-bold ${themeClasses.text}`}>Accent</p>
                 </div>
                 <div className="flex gap-2">
                   {['blue', 'purple', 'rose', 'amber'].map((c) => (
                     <button key={c} onClick={() => setAccent(c as any)} className={`w-8 h-8 rounded-full border-2 transition-all ${c === accent ? "border-white shadow-lg scale-110" : "border-transparent opacity-50"} bg-${c === 'amber' ? 'amber-500' : c + '-600'}`} />
                   ))}
                 </div>
               </div>
            </div>
          ...
          <button onClick={() => signOut({ callbackUrl: "/login" })} className="w-full py-4 bg-red-500/10 border border-red-500/20 text-red-500 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2">
  ...
  if (view === 'LEAVE_LIST') {
    return (
      <div className="min-h-[80vh] pb-24">
         ...
         {/* Placeholder for history - in real app, fetch from API */}
        ...
        <form onSubmit={handleSubmitLeave} className={`p-6 rounded-3xl border shadow-sm space-y-6 ${themeClasses.card} ${themeClasses.border}`}>
           ...
           <div className="space-y-2">
              ...
              <textarea 
                ...
                className={`w-full rounded-xl p-4 text-sm font-bold outline-none resize-none ${darkMode ? "bg-slate-700 text-white placeholder:text-slate-500" : "bg-slate-50 text-slate-900 placeholder:text-slate-400"}`}
           ...
           <button 
             ...
             className={`w-full py-4 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg active:scale-95 transition-transform disabled:opacity-50 disabled:cursor-not-allowed bg-${accent}-600`}
           >
