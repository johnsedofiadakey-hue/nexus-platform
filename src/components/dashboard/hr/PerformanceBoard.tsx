    ...
    <div className="p-8 space-y-8 h-full flex flex-col bg-white">
      ...
      <div className="flex items-center justify-between">
        <div className="flex flex-col">
           <span className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Operations Intelligence</span>
           <span className="text-[9px] font-bold text-blue-500 uppercase mt-0.5">Live Satellite Feed</span>
        </div>
        ...
        <div className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100 group hover:border-blue-200 transition-all">
           <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2 text-emerald-600">
                 <Users size={14} />
                 <span className="text-[9px] font-black uppercase tracking-widest">Footfall</span>
              </div>
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between px-2">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Grid Drift Pattern (7D)</h4>
          </div>
          <span className="text-[9px] font-black text-rose-600 bg-rose-50 px-2 py-0.5 rounded-full">
            {geofenceStats.reduce((acc: number, g: any) => acc + (g.breaches || 0), 0)} Breaches
          </span>
        </div>
                  ...
                  if (active && payload && payload.length) {
                    return (
                      <div className="bg-white p-3 rounded-2xl shadow-2xl border border-slate-100 animate-in zoom-in">
                        <p className="text-[9px] font-black text-rose-500 uppercase">{payload[0].value} Unauthorized Exits</p>
                      </div>
                    );
                  }
                  return null;
