  ...
  if (loading && feed.length === 0) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-slate-400 gap-3">
        ...
        <p className="text-xs font-black uppercase tracking-widest">Initializing Feed...</p>
      </div>
    );
  }
            ...
            feed.map((item) => (
              <div key={item.id} className="flex gap-4 p-3 rounded-2xl hover:bg-slate-50 transition-colors group">
                 <div className={`mt-1 w-2 h-2 rounded-full shrink-0 ${
                    item.type === 'SALE' ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.4)]' : 'bg-blue-500'
                 }`} />
